...
export default function Classrooms() {
  ...
  const fetchClassrooms = async () => {
    try {
      ...
      const response = await fetch(`/api/classrooms?${queryParams}`);
  ...
  return (
    <DashboardLayout>
      <Head>
        ...
        <meta name="description" content="Manage classrooms" />
      ...
      <div>
        <div className="flex justify-between items-center mb-6">
          ...
          <Link href="/classrooms/create" className="bg-blue-600 text-white py-2 px-4 rounded hover:bg-blue-700">
            Add New Classroom
          </Link>
        </div>
        ...
        {loading ? (
          <p className="text-center py-4">Loading classrooms...</p>
        ) : classrooms.length === 0 ? (
          <p className="text-center py-4">No classrooms found. Add a new classroom to get started.</p>
        ) : (
                ...
                <tbody>
                  {classrooms.map((classroom) => (
                    <tr key={classroom.id} className="hover:bg-gray-50">
                      ...
                      <td className="py-2 px-4 border-b">{classroom.teacher}</td>
                ...
                <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                  <div>
                    <p className="text-sm text-gray-700">
                      Showing <span className="font-medium">{classrooms.length > 0 ? (pagination.page - 1) * pagination.limit + 1 : 0}</span> to <span className="font-medium">{Math.min(pagination.page * pagination.limit, pagination.totalItems)}</span> of{' '}
