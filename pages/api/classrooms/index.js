...
async function getClassrooms(req, res) {
  try {
    ...
    if (search) {
      query = query.or(`name.ilike.%${search}%,teacher.ilike.%${search}%`);
    }
...
async function createClassroom(req, res) {
  try {
    ...
    if (error) {
      console.error('Supabase error details:', JSON.stringify(error, null, 2));
      return res.status(500).json({ message: 'Failed to save classroom data', error: error.message });
    }

    return res.status(201).json({ message: 'Classroom created successfully', data });
  } catch (error) {
    console.error('Server error details:', error.toString(), error.stack);
    return res.status(500).json({ message: 'Internal server error', error: error.message });
  }
