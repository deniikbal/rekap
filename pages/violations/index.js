...
export default function Violations() {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      // Remove the deleted violation from the state
  ...
  return (
    <DashboardLayout>
      <Head>
        ...
        <meta name="description" content="Manage violations" />
      ...
      <div>
        <div className="flex justify-between items-center mb-6">
          ...
          <Link href="/violations/create" className="bg-blue-600 text-white py-2 px-4 rounded hover:bg-blue-700">
            Add New Violation
          </Link>
        </div>
        ...
        {loading ? (
          <p className="text-center py-4">Loading violations...</p>
        ) : violations.length === 0 ? (
          <p className="text-center py-4">No violations found. Add a new violation to get started.</p>
        ) : (
                ...
                <tbody>
                  {violations.map((violation) => (
                    <tr key={violation.id} className="hover:bg-gray-50">
                      <td className="py-2 px-4 border-b">{violation.name}</td>
                      <td className="py-2 px-4 border-b">{violation.point}</td>
                      <td className="py-2 px-4 border-b">{violation.categories?.name || 'N/A'}</td>
                      <td className="py-2 px-4 border-b text-center">
                        <div className="flex justify-center space-x-2">
                          ...
                          <Link 
                            href={`/violations/edit/${violation.id}`}
                ...
                <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                  <div>
                    <p className="text-sm text-gray-700">
                      Showing <span className="font-medium">{violations.length > 0 ? (pagination.page - 1) * pagination.limit + 1 : 0}</span> to <span className="font-medium">{Math.min(pagination.page * pagination.limit, pagination.totalItems)}</span> of{' '}
